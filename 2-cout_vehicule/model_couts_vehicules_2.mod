...
var x1 {i in 1..m, j in 1..n} >=0; # la quantité livré du dépôt i vers le client j.
# Pas besoins de préciser integer ici : par propriété les solutions seront entières.
# Par camion 1

var x2 {i in 1..m, j in 1..n} >=0; # la quantité livré du dépôt i vers le client j.
# Pas besoins de préciser integer ici : par propriété les solutions seront entières.
# Par camion 2
...
number_of_truck2 {i in 1..m, j in 1..n} : N2[i,j]*C2 >= x2[i,j];
...
{
      printf "Client %d:\n", j;
      printf {i in 1..m: x1[i,j] != 0 or x2[i,j] != 0} "   Depot %d : %d m3, %d camions 1 et %d camions 2\n", i, (x1[i,j] + x2[i,j]), N1[i,j], N2[i,j]; 
}
